class TasksController < ApplicationController
  ...
  def index
    label_id = Label.find_by(id:params[:label_search]).id if params[:label_search].present?
    task_ids = TaskLabel.where(label_id:label_id).pluck(:task_id).to_a
    @tasks =
    if params[:search] == "true"
      if params[:label_search].present?
        current_user.tasks.where(id:task_ids).page(params[:page]).per(12)
      else
